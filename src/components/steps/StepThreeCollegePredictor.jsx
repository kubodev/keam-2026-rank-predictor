...
const StepThreeCollegePredictor = ({ predictedRank, predictions, onBack, onRecalculate }) => {
  ...
  const branchOptions = useMemo(
    () => [
      ALL_BRANCHES,
      ...Array.from(new Set(predictions.map((item) => item.branch))).sort((a, b) => a.localeCompare(b))
    ],
    [predictions]
  );
  const filteredPredictions = useMemo(
    () =>
      selectedBranch === ALL_BRANCHES
        ? predictions
        : predictions.filter((item) => item.branch === selectedBranch),
    [predictions, selectedBranch]
  );
  const visiblePredictions = filteredPredictions.slice(0, 60);
  const hiddenCount = Math.max(0, filteredPredictions.length - visiblePredictions.length);
        ...
        {predictions.length > 0 ? (
          <>
            <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4">
              <div className="grid gap-4 md:grid-cols-[1fr,280px] md:items-end">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-semibold text-navy">
                      {filteredPredictions.length.toLocaleString()} matching options
                    </span>
            ...
            {visiblePredictions.length > 0 ? (
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-6 text-center">
                <p className="font-heading text-2xl text-navy">No matches for this branch</p>
                <p className="mt-2 text-sm text-subtext">Choose another branch to see matching colleges for this rank.</p>
              </div>
            )}
